...
data Model : (env : List (String, Type)) -> (0 es : List (Type -> Type)) -> (ret : Type) -> Type where
  MkModel : Prog (ObsReader env :: Dist :: es) a -> Model env es a

runModel : Model env es a -> Prog (ObsReader env :: Dist :: es) a
...
handleCore : {es : _} -> Env env -> Model env es a -> Prog (Observe :: Sample :: es) a
