...
data Union : (xs : List a) -> Type where
  U : (ix : Elem x xs) => (x : a) -> Union xs

drop : (xs : List a) -> Elem x xs -> List a
drop (x :: xs) Here        = xs
drop (x :: xs) (There prf) = x :: drop xs prf

(-) : (xs : List a) -> (x : a) -> (prf : Elem x xs) => List a
